...
const Price = ({ value }) => {
    ...
    return (
        <p className="text-[1.5rem] font-bold leading-none text-apple-text">
            <span className="align-top text-[0.9375rem]">$</span>
            {dollars}
            <span className="align-top text-[0.9375rem]">.{cents}</span>
        </p>
    );
};
...
const ProductCard = ({ product, onAddToCart, variant = 'priceDrop' }) => {
    ...
    return (
        <article className="flex h-full w-[270px] shrink-0 flex-col rounded-2xl border border-black/[0.06] bg-white p-4 transition-shadow duration-200 ease-smooth hover:shadow-surface md:w-[285px]">
            <Link
            ...
            >
                <span className="block overflow-hidden rounded-xl bg-surface">
                    <img
                        ...
                        loading="lazy"
                        decoding="async"
                        className="mx-auto block h-[180px] w-auto object-contain p-3 transition-transform duration-300 ease-smooth group-hover:scale-[1.03]"
                    />
                </span>

                {isPriceDrop && saving > 0 && (
                    <span className="mt-3 inline-flex w-fit items-center gap-1 rounded-md bg-brand-red px-2 py-1 text-[0.8125rem] font-bold text-white [&_svg]:!text-[14px]">
                        <TrendingDownRoundedIcon aria-hidden="true" />
                        Price drop
                    </span>
                )}

                <h3 className={`${isPriceDrop && saving > 0 ? 'mt-2' : 'mt-3'} text-[1rem] font-bold leading-tight text-apple-text group-hover:text-brand-red`}>
                    {product.productName}
                </h3>

                {specs && (
                    <p className="mt-1 text-[0.875rem] font-normal leading-snug text-apple-gray">
                        {specs}
                    </p>
                )}
                ...
                {score > 0 && (
                    <span className="mt-2 flex items-center gap-1.5">
                        <Stars score={score} />
                        <span className="text-[0.875rem] font-medium text-apple-text">{score.toFixed(1)}/5</span>
                        {reviews > 0 && (
                            <span className="text-[0.875rem] font-normal text-apple-gray">({reviews.toLocaleString()})</span>
                        )}
                    </span>
                )}
                ...
                <span className="mt-3 block">
                    ...
                    {isPriceDrop && saving > 0 && (
                        <>
                            <span className="mt-1 block text-[0.875rem] font-bold text-brand-red">
                                Save ${money(saving)}
                            </span>
                            {/* "Was", not the reference's "Last lowest price":
                                originalPrice is the list price on the product,
                                not a tracked price history. */}
                            <span className="mt-0.5 flex items-center gap-1 text-[0.8125rem] font-normal text-apple-gray">
                                Was ${money(was)}
                    ...
                    {!isPriceDrop && was > now && (
                        ...
                        <span className="mt-1 block text-[0.875rem] font-normal text-apple-gray">
                            <s>${money(was)}</s> new
                        </span>
                    )}
                </span>
            ...
            <button
                ...
                className="mt-3 flex h-11 w-full items-center justify-center gap-1.5 rounded-lg border border-solid border-black/[0.12] bg-white text-[0.9375rem] font-bold text-apple-text outline-none transition-colors duration-200 ease-smooth hover:border-apple-text hover:bg-apple-bg focus-visible:ring-2 focus-visible:ring-brand-red focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:text-apple-gray disabled:hover:border-black/[0.12] disabled:hover:bg-white [&_svg]:!text-[18px]"
            >
