...
const FaqAccordion = () => {
    ...
    return (
        <section aria-labelledby="faq-heading" className="py-10 md:py-12">
            <div className="site-shell">
                <h2 id="faq-heading" className="text-[1.625rem] font-bold leading-tight tracking-[-0.02em] text-apple-text md:text-[2rem]">
                    Popular searches
                </h2>
                    ...
                    {FAQS.map((faq, index) => {
                        ...
                        return (
                            <li key={faq.q} className="border-b border-solid border-black/[0.08]">
                                <h3 className="m-0 text-[1.0625rem]">
                                    <button
                                        type="button"
                                        id={buttonId}
                                        aria-expanded={open}
                                ...
                                {open && (
                                    <div id={panelId} role="region" aria-labelledby={buttonId} className="px-1 pb-5">
                                        ...
                                        {faq.to && (
                                            <Link
                                                to={faq.to}
                                                className="mt-2 inline-block text-[0.9375rem] font-bold text-apple-text underline underline-offset-2 outline-none hover:text-brand-red focus-visible:ring-2 focus-visible:ring-brand-red focus-visible:ring-offset-2"
                                            >
                                                {faq.linkLabel}
                                            </Link>
                                        )}
